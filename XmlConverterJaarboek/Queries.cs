...
namespace XmlConverterJaarboek
{
    class Queries
    {
        public static string DOCTORS_FOR_INEXTENSO =
            ...
            Properties.Settings.Default.InExtensoNew + " FROM [" + Properties.Settings.Default.TableName + "]) AS x " +
        ...
        public static string DOCTORS_FOR_INEXTENSO_PERPROVINCE =
            "SELECT x.NOM, x.PRENOM, x.NoINAMI, x.Poste, x.Commune, x.CSouMS, x.ProvArrond, x.InExtensoNew FROM(SELECT *, " + Properties.Settings.Default.InExtensoNew + " FROM [" + Properties.Settings.Default.TableName + "]) AS x " +
        ...
        public static string DOCTORS_INTERNAL_FOR_COMPETENCE_PERPOSTAL =
            "SELECT x.NOM, x.PRENOM, x.Poste, x.Commune FROM(SELECT *, IIF(SPECIALITE = \"ONCO_MED\", \"ONCO_MED\", Compétence2) AS Competence2Real FROM [" + Properties.Settings.Default.TableName + "]) AS x " +
